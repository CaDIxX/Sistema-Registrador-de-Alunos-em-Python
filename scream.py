...
# Criando Frames
frame_logo = Frame(janela,width=950, height=100, bg=co6)
frame_logo.grid(row=0, column=0, pady=0, padx=0, sticky=NSEW, columnspan=5)

frame_botoes = Frame(janela,width=300, height=300, bg=co1, relief=RAISED)
frame_botoes.grid(row=1, column=0, pady=1, padx=0, sticky=NSEW)

frame_detalhes = Frame(janela,width=800, height=300, bg=co1, relief=RAISED)
frame_detalhes.grid(row=1, column=1, pady=1, padx=10, sticky=NSEW)

frame_tabela = Frame(janela,width=800, height=300, bg=co1, relief=RAISED)
...
def procurar():
    ...
    data_nascimento.insert(0, dados[5])
...
l_sexo.place(x=149, y=130)
c_sexo = ttk.Combobox(frame_detalhes, width=7, font=('Ivy 8 bold'), justify='center')
c_sexo['values'] = ('M', 'F', 'Outros')
c_sexo.place(x=150, y=160)

l_data_nascimento = Label(frame_detalhes, text="Data de nascimento *", anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
l_data_nascimento.place(x=290, y=10)
data_nascimento = DateEntry(frame_detalhes, width=18, justify='center', backgroud='darkblue', foreground='white', borderwidth=2, year=2023)
data_nascimento.place(x=300, y=40)

l_endereco = Label(frame_detalhes, text="Endereço *", anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
l_endereco.place(x=290, y=70)
e_endereco = Entry(frame_detalhes, width=15, justify='left', relief='solid')
e_endereco.place(x=300, y=100)

cursos = ['CCO', 'ADS', 'ENG. COMP', 'SI', '(...)']

l_curso = Label(frame_detalhes, text="Cursos *", anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
l_curso.place(x=290, y=130)
c_curso = ttk.Combobox(frame_detalhes, width=15, font=('Ivy 8 bold'), justify='center')
c_curso['values'] = (cursos)
c_curso.place(x=300, y=160)
...
botao_carregar = Button(frame_detalhes, command=escolher_imagem, text='Carregar Foto'.upper(), width=20, compound=CENTER, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co1, fg=co0)
...
l_nome.grid(row=0, column=0,columnspan=2, pady=(0, 5), padx=0, sticky=NSEW)
e_procurar = Entry(frame_procurar, width=15, justify='center', relief='solid', font=('Ivy 10'))
e_procurar.grid(row=1, column=0, pady=10, padx=5, sticky=NSEW)
...
botao_procurar.grid(row=1, column=1, pady=10, padx=5, sticky=NSEW)
...
app_adicionar = Button(frame_botoes,command=adicionar,image=app_img_adicionar,relief=GROOVE ,text=' Adicioar', width=100, compound=LEFT, overrelief=RIDGE, font=('Ivy 11 bold'), bg=co1, fg=co0)
...
app_atualizar = Button(frame_botoes,image=app_img_atualizar,command=atualizar,relief=GROOVE ,text='Atualizar', width=100, compound=LEFT, overrelief=RIDGE, font=('Ivy 11 bold'), bg=co1, fg=co0)
...
app_deletar = Button(frame_botoes,image=app_img_deletar,relief=GROOVE ,command=deletar,text='  Deletar', width=100, compound=LEFT, overrelief=RIDGE, font=('Ivy 11 bold'), bg=co1, fg=co0)
app_deletar.grid(row=3, column=0, pady=5, padx=10, sticky=NSEW)

# linha
l_linha = Button(frame_botoes,image=app_img_deletar,relief=GROOVE ,text='h', width=1, height=450,anchor=NW,  font=('Ivy 1'), bg=co1, fg=co1)
l_linha.place(x=251, y=15)
